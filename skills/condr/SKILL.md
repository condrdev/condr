...
- A `blocked` agent's `blocked_on` (also on `agent list` and `pane list`) says what it waits for when its hooks told Condr: `Bash: cargo test` is a permission to approve in the Pane, a question text is a question to answer. `null` means look at the Pane.
