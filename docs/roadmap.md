...
- 在既有 SSH tunnel 验证稳定后，已增加原生 `ssh://` 连接项：Client 通过系统 `ssh` 的 stdio 连接远端 `condr server bridge`，bridge 只负责连接远端私有 endpoint 并双向转发现有协议；远端 Server 仍独立存活。保留 Local/TCP，首版要求远端预装 `condr` 且 Server 已启动，支持 `?bin=/absolute/path` 指定可执行文件，不做自动安装、升级或 live handoff；远端 Server 自动启动留到后续 install 安装流程一起实现。设计依据见 [Herdr SSH remote 调查](research/herdr-ssh-v0.8.2.md)。
