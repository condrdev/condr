...
`condr-core` and `condr-server` run Tokio; `condr-gui` uses the GPUI executor. The wire protocol is transport-independent: length-prefixed protobuf frames (ADR 0028), a `Hello`/`Welcome` handshake, protocol-range negotiation, and bounded frame sizes. Local connections use a private `interprocess` endpoint (Unix domain socket or Windows named pipe); remote connections use a TCP endpoint, often a Server loopback listener forwarded through an external SSH TCP tunnel, and every TCP connection authenticates both static keys and encrypts with WireGuard-style `Noise_IKpsk2` (ADR 0011). Both paths carry the same protocol and server behavior; the local Server does not expose a public listener, and capability-based authorization is deferred.

The Server continues consuming PTY output and updating VT state with no clients connected. Reconnecting to a running Server first receives a one-shot authoritative bootstrap (stable Server identity plus runtime epoch, Session structure, Pane focus, cwd, each Pane's live terminal view, foreground shell/Agent identity and status), then subscribes to ordered reliable events and a coalesced terminal visual stream; no shell or Agent is recreated. Each Client retains its own View across reconnects and reconciles it against the received structure (ADR 0021). The identity/epoch pair lets the GUI distinguish a live reconnect from a replacement Server after restart. The server owns the event order and is the only authority for layout mutations. MVP has one active controller per Server/Session; collaborative multi-client control is not guaranteed. This connection synchronization is distinct from durable Session Snapshot restore after a Server restart.
...
- The sidebar is persistent and resizable. Selecting a Server shows the Workspace this Client last viewed on it, falling back to the first Workspace or Start Page. Selecting an Agent shows its Workspace and Tab locally and targets its Pane; a controlling Client also updates the Tab's Pane focus on the Server.
- The Workspace this Client shows owns the Tab row. Panes have no permanent title bar; a visible focus treatment identifies the focused Pane, while Pane commands remain available through shortcuts and context menus.
...
- Workspace and Tab navigation updates this Client's View immediately and sends no Layout command, including for viewing-only Clients. A `LayoutChanged` event updates structure in place and reconciles removed targets. A creation or viewer-opening command moves only the requesting Client when its `LayoutApplied` identifies the result. Explicit CLI activation broadcasts `Activated` to every viewer. Cached Dock state and the Client's View are never Session or persistence truth.
...
- Restarting a Server restores Workspace/Tab/Pane structure, names, layout, order, and cwd using fresh shells, then resumes the saved hook-reported native conversation IDs. Observed process exits clear their IDs; Server shutdown saves the existing IDs without an extra process check. Closing all Workspaces keeps the Server Session empty and the connected GUI on Start Page.
...
- Local and remote Servers have identical product behavior and wire semantics. The remote path is a TCP endpoint, often through an external SSH TCP tunnel; each connection is authenticated and encrypted by the Server itself (ADR 0011), while capability-based authorization, account management, and public Internet exposure are deferred. Servers use private local endpoints by default.
...
- GUI reconnect preserves the authoritative Session/layout plus live processes and terminal state held by a running Server, including a running Agent and its visible Pane. Server restart restores structure with fresh shells and native conversation references; terminal history, live processes, commands, and Agent state are not restored. Conversation contents remain in the native CLI’s storage.
- Agent state and conversation IDs come from installed hooks (ADR 0014); unsupported or silent CLIs stay `unknown`. Native resume reopens the saved conversation without replaying prompts or restoring arbitrary CLI launch arguments.
...
- Paths travel and persist as UTF-8 strings. A non-UTF-8 Unix cwd observation is rejected and the last durable Pane cwd is retained.
