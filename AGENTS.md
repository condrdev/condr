...
- **Transport: versioned binary protocol over transport adapters.** Frames are Protocol Buffers (`prost`), varint length-delimited, from the `.proto` files in `proto/condr/v1/`, which `condr-core`'s `build.rs` compiles with `protox`; generated types stay inside `protocol::pb` and convert to the domain types at the boundary (ADR 0028). The handshake (`protocol::handshake`: `ClientHandshake`, `Welcome`, `Refusal`) is frozen by field number and carries each binary's `build_identity()`; a different build only warns (ADR 0027). The protocol is a range, not an equality: each side names the oldest peer protocol it works with, fields and oneof members are only added, a newer peer's unknown enum value decodes to its documented fallback and an unknown oneof member makes the enclosing message unknown, never a disconnect (ADR 0028). Locally, `interprocess` Unix domain sockets / Windows named pipes; remotely, the same protocol over a TCP endpoint. Every TCP connection is authenticated and encrypted with WireGuard-style `Noise_IKpsk2` (`snow`) using mutual static keys; new devices pair with a one-time invite (ADR 0011). Capability-based authorization comes later. SSH connections run the system `ssh -T` to execute `condr server bridge` remotely and forward the remote private socket; SSH handles auth and encryption (ADR 0015), and on Unix Condr adds OpenSSH connection sharing unless the user's own config already names a `ControlPath`. The `condr` CLI reaches a saved Device directly with `--device <name>` from the same `[[client.servers]]` list the GUI writes; no Server proxies another (ADR 0022). The Server only exposes a local private endpoint by default and never listens on the public internet.
- **Persistence: protobuf** for the Session Snapshot and the GUI state file (ADR 0028), **TOML** for config. Every `config.toml` read and write goes through `condr_core::{read_config_value, update_config_values}` (one cross-process lock, format-preserving edits); no crate parses or writes the file on its own.
- **Workspace-relative paths: `relative-path`.** Every path the protocol, Session and Git layer name relative to a Workspace root is a `RelativePathBuf`: `/`-separated on every platform, so a Windows client and a Linux Server agree. Only the Server turns one into a native path with `to_path(root)`; absolute paths such as the Workspace root stay `PathBuf`.
...
- **Agent state detection: only from the agent CLI's own hooks** (ADR 0014). The process table identifies the agent; `condr agent-hook` writes hook events back to the controlling terminal as OSC 777; the Server intercepts them ahead of the VT and drives `AgentState`. Nothing reported means `Unknown`; there is no screen-text classification. A `Blocked` state carries what the hook said it waits for as `blocked_on`, shown in the sidebar, the OS notification, the CLI JSON and the GUI's "Needs you" list (ADR 0024). The GUI layers "done" on top.
...
- **Logging: `tracing`** (ADR 0019). `condr-core` takes only the facade; the subscriber, daily rolling non-blocking file appender and panic hook live in `condr-server`'s `logging` module, which `condr server run` and the GUI both call. `CONDR_LOG` is an `EnvFilter` string. Per-chunk and per-frame paths (PTY reads, VT parsing, frame publication, `prepaint`/`paint`) carry `debug!`/`trace!` only; `info!` and above belong to lifecycle, Agent state, recovery, configuration and errors. No `thiserror`/`anyhow`.
- **Errors are shown where they belong** (ADR 0020). A connection state lives on its Device: the sidebar mark, a dedicated page when nothing is open, a centred pill over a Workspace after a two-second grace. A failed operation is one `Condr::report_error` toast with Copy. A validation failure sits under its field; dialog callbacks return `Result<(), String>`. "Viewing only" is a mode in muted text, never red. Nothing goes into a shared error string, and the Welcome page never carries a failure.
- **Dependencies: declare only `gpui-kit = "0.6"` from crates.io.** Use `gpui_kit::*` for GPUI types, `gpui_kit::component` for components, `gpui_kit::assets` for assets; create the app with `gpui_kit::application()` and initialize with `gpui_kit::init(cx)`. The matching `gpui-pre` crates are managed by Kit and pinned by `Cargo.lock`; do not declare GPUI/platform/component/asset crates directly. The one exception is `gpui-pre-reqwest-client`, which Kit builds but does not re-export; the GUI sets it as GPUI's HTTP client for the update check (ADR 0029). GUI tests use `gpui-kit/test-support`.
