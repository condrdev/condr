...
try {
  ...
  for (const agent of ["pi", "omp"]) {
    ...
    if (agent === "pi") {
      await emit("ui_prompt_start", { kind: "confirm", reason: "ui_prompt", title: "Overwrite?" })
      assert.equal(last(), "question-asked")
      assert.equal(reports().at(-1).detail, "Overwrite?", "the dialog title is the detail")
      ...
      for (const toolCallId of ["a", "b"]) await emit("tool_approval_requested", { sessionId: id, toolCallId, toolName: toolCallId === "a" ? "bash" : "write" })
      assert.equal(reports().at(-1).detail, "write", "the newest approval names its tool")
      ...
      assert.equal(reports().at(-1).detail, "write")
      ...
      await emit("tool_execution_start", { toolName: "ask", toolCallId: "q", input: { question: "Deploy?" } })
      assert.equal(last(), "question-asked")
      assert.equal(reports().at(-1).detail, "Deploy?", "the ask tool carries its question")
